	...
	return (
		...
		>
			<SliderPrimitive.Track className='relative h-2 w-full grow overflow-hidden rounded-full bg-white'>
				<SliderPrimitive.Range className='absolute h-full bg-blue-primary' />
			</SliderPrimitive.Track>
			<>
				{initialValue.map((value, index) => (
					<React.Fragment key={index}>
						<SliderPrimitive.Thumb className='relative block h-4 w-4 rounded-full border-2 border-primary bg-white ring-offset-blue-primary transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50'>
									...
									{/* {lableContenPos === 'left' && (
										<>
											{typeof label === 'function' ? (
												<span className='inline-block  -translate-y-0.8'>{label(value)}</span>
											) : (
												label && <span className='inline-block translate-y-1/2'>{label}</span>
											)}
										</>
									)} */}
									<NumberFlow
